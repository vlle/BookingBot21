...
from app.handlers.main_menu import main_menu_keyboard


async def cmd_start(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(
        "Чтобы забронировать место, нажмите /book",
...
def register_handlers_common(dp: Dispatcher, admin_id: int):
    ...
    dp.register_message_handler(secret_command, IDFilter(
        user_id=admin_id), commands="abracadabra")
